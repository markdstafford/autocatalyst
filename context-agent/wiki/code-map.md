...
- `packages/execution/` — execution-plane package. Public entry point: `src/index.ts`, which exports the runner scaffold plus workspace provisioning, prune, and teardown APIs from `src/workspace.ts` (`provisionWorkspace`, `pruneWorkspacePath`, `teardownWorkspace`, lifecycle result/error types, and redacted workspace diagnostics). Internal workspace modules are package-private: `src/internal/workspace-paths.ts` owns shape selection, safe segment validation, path resolution, containment checks including no-final-symlink-resolution prune containment, and branch-name derivation; `src/internal/workspace-driver.ts` owns argument-array git commands, host clone/fetch/default-branch/worktree operations, stat classification, checkpoint/branch operations, credential redaction, and guarded filesystem operations; `src/internal/workspace-pruner.ts` is the single policy-level gateway for workspace filesystem deletion and git worktree removal; `src/internal/workspace-teardown.ts` applies terminal-state retention policy; `src/internal/workspace-provisioner.ts` orchestrates no-workspace, scratch-only, and two-root provisioning plus pruner-mediated rollback; `src/internal/workspace-logger.ts` provides the injectable structured logging seam used by pruner and teardown. Tests: `src/workspace-paths.spec.ts`, `src/workspace-provisioner.spec.ts`, `src/workspace-pruner.spec.ts`, `src/workspace-teardown.spec.ts`, `src/workspace.integration.spec.ts`, and `src/workspace-teardown.integration.spec.ts`. Useful targeted commands: `pnpm nx test execution -- workspace-paths.spec.ts`, `pnpm nx test execution -- workspace-provisioner.spec.ts`, `pnpm nx test execution -- workspace-pruner.spec.ts`, `pnpm nx test execution -- workspace-teardown.spec.ts`, `pnpm nx test execution -- workspace.integration.spec.ts`, `pnpm nx test execution -- workspace-teardown.integration.spec.ts`, `pnpm nx build execution`, `pnpm nx lint execution`.
