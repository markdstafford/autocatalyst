...
// Production source files/directories allowed to contain both gh pr commands AND executeGh/GhExec* usage.
// To add a new adapter that legitimately uses gh pr commands, add its src/ path here.
const FULLY_ALLOWED_PATHS = [
...
// Files allowed to reference executeGh/GhExec* symbols (injectable seam) but NOT direct gh pr commands.
const EXEC_SYMBOL_ALLOWED_PATHS = [
...
function isFullyAllowed(relative) {
  return FULLY_ALLOWED_PATHS.some(allowed => relative.startsWith(allowed) || relative === allowed);
}

function isExecSymbolAllowed(relative) {
  return EXEC_SYMBOL_ALLOWED_PATHS.some(allowed => relative.startsWith(allowed) || relative === allowed);
}
...
for (const file of allFiles) {
  ...
  if (isFullyAllowed(relative)) continue;

  const content = readFileSync(file, 'utf8');

  // Files in EXEC_SYMBOL_ALLOWED_PATHS may reference the executeGh seam but must not contain direct gh pr strings.
  const execSymbolAllowed = isExecSymbolAllowed(relative);

  const hasGhPr = GH_PR_PATTERNS.some(p => content.includes(p));
  const hasExecSymbol = !execSymbolAllowed && EXEC_PATTERNS.some(p => content.includes(p));
