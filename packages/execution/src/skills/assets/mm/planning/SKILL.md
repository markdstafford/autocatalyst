...
5. **Spec completion** — after the task list is approved and written to disk, stop and return control to Autocatalyst (see Autocatalyst handoff boundary below)
...
1. **Check for required ADRs** — verify foundational decisions exist in `{docs_root}/adrs/`
2. **Check wiki documents** — verify domain model, database schema, API contracts, and design system exist in `{docs_root}/wiki/`
3. **Check dependent features** — verify required feature specs exist in `{docs_root}/specs/`
...
Wiki documents in `{docs_root}/wiki/` are the source of truth. When any stage produces new:
...
All artifacts are stored in `{docs_root}/`:
...
**Frontmatter**: All artifact files begin with a YAML frontmatter block. The `status` field reflects the artifact's lifecycle state. Skills populate and update frontmatter fields at each lifecycle event (creation, approval). See the technical changes in `enhancement-spec-lifecycle-state.md` for the full schema per file type.
...
`{docs_root}/.friction-logs/` is created by the `mm:friction-log` skill and must be gitignored. It contains friction log sessions and is not committed to the repository.
...
## Autocatalyst handoff boundary

After the approved authoring artifact is written and the task list is complete, stop and return control to Autocatalyst. Autocatalyst owns the session lifecycle, including any handoff steps that follow spec authoring. Do not instruct the agent to perform any session-lifecycle operations inside this runtime skill.
