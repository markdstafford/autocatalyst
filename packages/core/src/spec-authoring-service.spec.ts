...
describe('completeSpecAuthoring side effects', () => {
  ...
  it('recovers existing artifact without write or commit when existing file content is unchanged', async () => {
    ...
    expect(deps.filesystem.readFile).toHaveBeenCalledWith({
      workspaceRepoRoot: '/tmp/repo',
      relativePath: 'context-human/specs/feature-artifact-feedback-gate.md'
    });
    expect(deps.filesystem.writeFile).not.toHaveBeenCalled();
  ...
  it('rewrites, validates, and commits when an existing artifact file differs from revised content', async () => {
    const calls: string[] = [];
    const existingArtifact = {
      id: 'artifact_existing',
      runId: 'run_1',
    ...
    const deps = makeDeps({
      filesystem: {
        ...
        readFile: vi
          .fn()
          .mockImplementationOnce(async () => { calls.push('readExistingFile'); return previousMarkdown; })
          .mockImplementationOnce(async () => { calls.push('readWrittenFile'); return revisedMarkdown; })
      },
      git: {
        commitFiles: vi.fn(async () => { calls.push('commitFiles'); return {}; })
      },
      artifacts: {
        ...
        findById: vi.fn(),
        listByRun: vi.fn(),
        findByRunAndKind: vi.fn(async () => { calls.push('findByRunAndKind'); return existingArtifact; }),
        updateCachedStatus: vi.fn()
      }
    });
    ...
    expect(calls).toEqual(['findByRunAndKind', 'readExistingFile', 'writeFile', 'readWrittenFile', 'commitFiles']);
    expect(deps.filesystem.writeFile).toHaveBeenCalledWith({
      workspaceRepoRoot: '/tmp/repo',
      relativePath: 'context-human/specs/feature-artifact-feedback-gate.md',
      contents: revisedMarkdown
    });
    expect(deps.git.commitFiles).toHaveBeenCalledWith({
      workspaceRepoRoot: '/tmp/repo',
      relativePaths: ['context-human/specs/feature-artifact-feedback-gate.md'],
      message: 'docs: revise feature spec artifact-feedback-gate'
    });
    expect(output.artifactCreated).toBe('recovered');
    expect(output.artifact.id).toBe('artifact_existing');
    expect(deps.artifacts.create).not.toHaveBeenCalled();
  ...
  it('rejects an existing artifact whose location differs from the result path before file writes', async () => {
    const existingArtifact = {
      id: 'artifact_existing',
      runId: 'run_1',
      owner: { kind: 'human', id: 'user_1', tenantId: 'tenant_1' },
      ...
      kind: 'feature_spec',
      canonicalRecord: 'file',
      location: 'context-human/specs/feature-other-path.md',
      cachedStatus: 'draft',
      publicationRefs: [],
      createdAt: '2026-06-11T00:00:00.000Z',
      updatedAt: '2026-06-11T00:00:00.000Z'
    ...
    await expect(completeSpecAuthoring(makeInput(), deps)).rejects.toMatchObject({
      code: 'spec_artifact_location_mismatch'
    });
...
describe('completeSpecAuthoring feedback disposition', () => {
  ...
  it('addresses open artifact feedback after a changed existing spec is committed', async () => {
    const calls: string[] = [];
    const existingArtifact = {
      id: 'artifact_existing',
      runId: 'run_1',
    ...
    const feedbackData: Feedback = {
      ...
      title: 'Add failure mode',
      body: 'Add failure mode.',
      thread: [{ id: 'thread_original', author: { kind: 'human', id: 'user_1', tenantId: 'tenant_1' }, body: 'Add failure mode.', createdAt: now }],
      createdAt: now,
      updatedAt: now
    };
    const feedbackStore = new Map<string, Feedback>([['fb_artifact', feedbackData]]);
    const feedbackRepo: import('./feedback-lifecycle.js').FeedbackLifecycleDependencies['feedback'] = {
      create: vi.fn(),
      findById: async (id: string) => feedbackStore.get(id) ?? null,
      listByRun: async () => [...feedbackStore.values()],
      updateStatusAndAppendThread: async (input) => {
        calls.push('addressFeedback');
        const existing = feedbackStore.get(input.feedbackId);
        if (existing === undefined) throw new Error('not found');
        const updated = { ...existing, status: input.nextStatus, updatedAt: input.updatedAt };
        feedbackStore.set(input.feedbackId, updated as typeof existing);
    ...
    const deps = makeDeps({
      ...
      git: { commitFiles: vi.fn(async () => { calls.push('commitFiles'); return {}; }) },
      artifacts: {
        create: vi.fn(),
        findById: vi.fn(),
        listByRun: vi.fn(),
    ...
    expect(calls).toEqual(['findByRunAndKind', 'readExistingFile', 'writeFile', 'readWrittenFile', 'commitFiles', 'addressFeedback']);
    const updated = await feedbackRepo.findById('fb_artifact');
    expect(updated?.status).toBe('addressed');
  });

  it('does not address artifact feedback when a changed existing spec commit fails', async () => {
    const existingArtifact = {
      id: 'artifact_existing',
      runId: 'run_1',
      owner: { kind: 'human', id: 'user_1', tenantId: 'tenant_1' },
    ...
    const feedbackRepo: import('./feedback-lifecycle.js').FeedbackLifecycleDependencies['feedback'] = {
      ...
      findById: async (id: string) => feedbackStore.get(id) ?? null,
      listByRun: async () => [...feedbackStore.values()],
      updateStatusAndAppendThread: vi.fn(async () => { throw new Error('feedback should not be addressed'); }),
      appendThreadEntry: vi.fn()
    };
    const deps = makeDeps({
      filesystem: {
        ...
        readFile: vi
          .fn()
          .mockImplementationOnce(async () => previousMarkdown)
          .mockImplementationOnce(async () => revisedMarkdown)
      },
      git: { commitFiles: vi.fn(async () => { throw new Error('git failed'); }) },
      artifacts: {
        create: vi.fn(),
    ...
    await expect(completeSpecAuthoring(makeInput(), deps)).rejects.toMatchObject({ code: 'spec_commit_failed' });
    expect(feedbackRepo.updateStatusAndAppendThread).not.toHaveBeenCalled();
    const updated = await feedbackRepo.findById('fb_artifact');
